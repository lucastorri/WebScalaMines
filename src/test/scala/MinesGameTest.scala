...
class MinesGameTest extends Spec with ShouldMatchers {
    ...
    object GameSolver {
        implicit def minesGame2GameSolver(game: MinesGame) = new GameSolver(game)
    }
    class GameSolver(game: MinesGame) {
        def findBombs: List[(Int, Int)] = game.board.filter(_.mined).map(_.position)
        
        def findFreeSquares: List[Square] = game.board.filter(!_.mined)
            
        def solve: MinesGame = { findFreeSquares.foreach(_.reveal); game }
    }
    import GameSolver.minesGame2GameSolver
    ...
    describe("A Mines Game") {
        ...
        it("should be able to check if it was finished when lost") {
        	val game: MinesGame = new MinesGame(2)
        	val board = game.board
            var bombs = game.findBombs
        	evaluating { board.square(bombs(0)).reveal } should produce[BombExplodedException]
        	game should be('lost)
        	game should be('finished)
        }
        
        it("should be able to have a different number of mines other than default") {
            val totalMines = 4
            val game: MinesGame = new MinesGame(2, totalMines)
            game.board.map(_.mined).reduceLeft(_ && _) should be (true)
        }
        
        it("should be able to check if won with a different number of mines") {
            val game: MinesGame = new MinesGame(2, 3)
            game.solve should be ('won)
        }
    }
